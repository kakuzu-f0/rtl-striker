...
const char* const captive_portals[12] = {
    "Default",
    "Facebook",
    "Amazon",
    "Apple",
...
void rtl_striker_menu_callback(void* context, uint32_t index) {
    FURI_LOG_D(TAG, __func__);
    rtl_striker* app = context;
    FURI_LOG_I("rtl_striker", "index: %ld", index);
    switch(index) {
    case ScanWifi:
        FURI_LOG_I("rtl_striker", "scanWifis");
        scene_manager_handle_custom_event(app->scene_manager, ScanWifi);
        break;
    case ListWifis:
        FURI_LOG_I("rtl_striker", "listwifis");
        scene_manager_handle_custom_event(app->scene_manager, ListWifis);
        break;
    case CustomBeacon:
        FURI_LOG_I("rtl_striker", "customAP");
        scene_manager_handle_custom_event(app->scene_manager, CustomBeacon);
        break;
    case Help:
        FURI_LOG_I("rtl_striker", "Help");
        scene_manager_handle_custom_event(app->scene_manager, Help);
        break;
    case Exit:
        FURI_LOG_I("rtl_striker", "Exit");
...
void main_menu_scene_on_enter(void* context) {
    ...
    rtl_striker* app = context;
    menu_reset(app->menu);
    menu_add_item(app->menu, "Scan Wifi", &I_Scan_Wifi, ScanWifi, rtl_striker_menu_callback, app);
    menu_add_item(app->menu, "List Wifis", &I_List_Wifi, ListWifis, rtl_striker_menu_callback, app);
    menu_add_item(
        app->menu, "Send Beacon", &I_random_AP, CustomBeacon, rtl_striker_menu_callback, app);
    menu_add_item(app->menu, "Help", &I_help, Help, rtl_striker_menu_callback, app);
    menu_add_item(app->menu, "Exit", &I_exit, Exit, rtl_striker_menu_callback, app);
    menu_set_selected_item(app->menu, app->menuIndex);
    view_dispatcher_switch_to_view(app->view_dispatcher, mainMenuView);

    UNUSED(context);
}
static void app_free(rtl_striker* app);
bool main_menu_scene_on_event(void* context, SceneManagerEvent event) {
    FURI_LOG_D(TAG, __func__);
    rtl_striker* app = context;

    FURI_LOG_I("rtl_striker", "%s event.type: %d", __func__, event.type);
    FURI_LOG_I("rtl_striker", "%s event.event: %ldd", __func__, event.event);
    switch(event.type) {
    case SceneManagerEventTypeCustom:
        ...
        case Help:
            ...
            break;
        default:
            FURI_LOG_W("rtl_striker", "Evento desconocido: %ld", event.event);
            break;
        }
        return true;
        break;
    case SceneManagerEventTypeBack:
        ...
        break;
    default:
        FURI_LOG_W("rtl_striker", "Tipo de evento desconocido: %d", event.type);
...
void scan_networks_scene_on_enter(void* context) {
    ...
    // Show loading with animated searching effect
    ...
    int times = 0;
    while(scanFinish == false && times < 40) { // 20 seconds with animation
        // Only update widget every 2 iterations to reduce flicker
        if(times % 2 == 0) {
            widget_reset(app->widget);
            widget_add_string_element(
                app->widget, 64, 20, AlignCenter, AlignTop, FontPrimary, 
                searching_texts[(times/2) % 4]);
            widget_add_string_element(
...
void list_networks_scene_on_enter(void* context) {
    ...
    for(int i = 0; i < app->wifiCount; i++) {
        ...
        if(temp_len < sizeof(temp) - 1) {
            strlcat(temp, channelSt, sizeof(temp));
        ...
        if(temp_len < sizeof(temp) - 1) {
            strlcat(temp, app->wifiList[i].APssid, sizeof(temp));
        }
...
void portal_type_change_call_back(VariableItem* item) {
    ...
    rtl_striker* app = variable_item_get_context(item);
...
void config_attack_scene_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        app->variableList, "Portal Type", 12, portal_type_change_call_back, app);
...
void help_scene_on_enter(void* context) {
    ...
    const char* help_text = 
        ...
        "Original Project:\n"
        "Gorebrau - delfyRTL\n"
        "github.com/gorebrau/delfyRTL\n"
        "\n"
        "Features:\n"
        "• Deauth Attacks\n"
        "• Evil Portal (12 templates)\n"
        "• Beacon Flood (3 modes)\n"
        "\n"
        "For educational\n"
        "purposes only!";
    
    widget_add_text_scroll_element(
        app->widget, 0, 0, 128, 64, help_text);
...
void (*const rtl_striker_scene_on_enter_handlers[])(void*) = {
...
bool (*const rtl_striker_scene_on_event_handlers[])(void*, SceneManagerEvent) = {
...
void (*const rtl_striker_scene_on_exit_handlers[])(void*) = {
...
static const SceneManagerHandlers rtl_striker_scene_manager_handlers = {
    .on_enter_handlers = rtl_striker_scene_on_enter_handlers,
    .on_event_handlers = rtl_striker_scene_on_event_handlers,
    .on_exit_handlers = rtl_striker_scene_on_exit_handlers,
    .scene_num = rtl_strikerSceneCount,
};

static bool rtl_striker_custom_callback(void* context, uint32_t custom_event) {
    FURI_LOG_D(TAG, __func__);
    furi_assert(context);
    rtl_striker* app = context;
...
bool rtl_striker_back_event_callback(void* context) {
    FURI_LOG_D(TAG, __func__);
    furi_assert(context);
    rtl_striker* app = context;
...
void uart_process_line(FuriString* line, void* context) {
    ...
    if(strncmp(furi_string_get_cstr(line), "AP:", 3) == 0) {
        ...
        if(app->wifiCount >= 50) { // Limit to prevent memory exhaustion
            FURI_LOG_W(TAG, "WiFi list limit reached (50)");
            return;
        }
        ...
        strlcpy(newAP->APssid, token, strlen(token) + 1);
        ...
        scanFinish = true;
        FURI_LOG_D(TAG, "SCAN FINISH - Found %d networks", app->wifiCount);
        ...
        if(!token) return;
        token = strtok(NULL, "|");
        if(token && strlen(credentials) + strlen(token) + 2 < sizeof(credentials)) {
            strlcat(credentials, token, sizeof(credentials));
            strlcat(credentials, "\n", sizeof(credentials));
...
static rtl_striker* app_alloc() {
    FURI_LOG_D(TAG, __func__);
    rtl_striker* app = malloc(sizeof(rtl_striker));
    if(!app) return NULL;
    
    // Initialize all fields to zero first
    memset(app, 0, sizeof(rtl_striker));
    ...
    memset(app->ssidAP, 0, app->ssidAPLength);
    ...
    app->deauthReason = 2;
    app->mac_type = 0;
    app->scene_manager = scene_manager_alloc(&rtl_striker_scene_manager_handlers, app);
    ...
    view_dispatcher_set_custom_event_callback(app->view_dispatcher, rtl_striker_custom_callback);
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, rtl_striker_back_event_callback);
...
int32_t rtl_striker_app(void* p) {
    FURI_LOG_D(TAG, __func__);
    UNUSED(p);

    rtl_striker* app = app_alloc();
